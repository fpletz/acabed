...
var ColorRoller = new Class({
        ...
        initialize: function(element,options){
                ...
                els.crColorRoller.inject($(element));
        ...
        addEvents: function(){
                ...
                els.crBar.addEvents({
                        ...
                        'mousedown':function(e){
                                mousedown(e);
                                self.slider(e);
                                var hex = self.getValues(['R','G','B']).rgbToHex().toUpperCase();
                                self.fireEvent('change', hex);
                        }
                });
        ...
        setRGB: function(val,step){
                ...
                if (step){
                        var hsv = val.rgbToHsb(); //fromRgb(this.space);
        ...
        setPicker: function(val,S,step){
                ...
                if (step){
                        var hsv = this.vLast
                                ? [val, S, this.val]
                                : [this.val, S, val];
                        this.setRGB(hsv.hsbToRgb()); //toRgb(this.space));
                }
        },
        setSlider: function(val,step){
                ...
                } else var bg = [val,100,100].hsbToRgb(); //toRgb();
                ...
                if (step) this.setRGB(this.getValues([0,'S',1]).hsbToRgb());
        },
